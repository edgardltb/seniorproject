...
abstract class Media implements ActiveRecordInterface
{
    ...
    /**
     * Sets the value of the [video] column.
    ...
    public function setVideo($v)
    {
        ...
        if ($this->video !== $v) {
            $this->video = $v;
            $this->modifiedColumns[MediaTableMap::COL_VIDEO] = true;
        }
    ...
    public function setLink($v)
    {
        ...
        if ($this->link !== $v) {
            $this->link = $v;
            $this->modifiedColumns[MediaTableMap::COL_LINK] = true;
        }
    ...
    public function hydrate($row, $startcol = 0, $rehydrate = false, $indexType = TableMap::TYPE_NUM)
    {
        try {
            ...
            $col = $row[TableMap::TYPE_NUM == $indexType ? 0 + $startcol : MediaTableMap::translateFieldName('MediaId', TableMap::TYPE_PHPNAME, $indexType)];
            $this->media_id = (null !== $col) ? (int) $col : null;

            $col = $row[TableMap::TYPE_NUM == $indexType ? 1 + $startcol : MediaTableMap::translateFieldName('Video', TableMap::TYPE_PHPNAME, $indexType)];
            $this->video = (null !== $col) ? (boolean) $col : null;
    ...
    protected function doInsert(ConnectionInterface $con)
    {
        ...
        if ($this->isColumnModified(MediaTableMap::COL_VIDEO)) {
            ...
            foreach ($modifiedColumns as $identifier => $columnName) {
                switch ($columnName) {
                    case 'Media_id':
                        ...
                        break;
                    case 'video':
                        $stmt->bindValue($identifier, (int) $this->video, PDO::PARAM_INT);
                        break;
                    case 'link':
    ...
    public function buildCriteria()
    {
        ...
        if ($this->isColumnModified(MediaTableMap::COL_VIDEO)) {
            $criteria->add(MediaTableMap::COL_VIDEO, $this->video);
        }
        if ($this->isColumnModified(MediaTableMap::COL_LINK)) {
