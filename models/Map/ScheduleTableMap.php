...
class ScheduleTableMap extends TableMap
{
     ...
     */
    const COL_MENTOR_ID = 'schedule.Mentor_id';

    /**
     * the column name for the Customer_id field
     */
    const COL_CUSTOMER_ID = 'schedule.Customer_id';

    /**
     * the column name for the room field
     */
    const COL_ROOM = 'schedule.room';
  ...
  array (
     ...
     * and findPk*() calls.
     *
     * @param \Schedule $obj A \Schedule object.
    ...
    public static function addInstanceToPool($obj, $key = null)
    {
        if (Propel::isInstancePoolingEnabled()) {
            if (null === $key) {
                $key = serialize([(null === $obj->getScheduleId() || is_scalar($obj->getScheduleId()) || is_callable([$obj->getScheduleId(), '__toString']) ? (string) $obj->getScheduleId() : $obj->getScheduleId()), (null === $obj->getMentorId() || is_scalar($obj->getMentorId()) || is_callable([$obj->getMentorId(), '__toString']) ? (string) $obj->getMentorId() : $obj->getMentorId()), (null === $obj->getCustomerId() || is_scalar($obj->getCustomerId()) || is_callable([$obj->getCustomerId(), '__toString']) ? (string) $obj->getCustomerId() : $obj->getCustomerId())]);
            } // if key === null
     ...
     *
     * @param mixed $value A \Schedule object or a primary key value.
     */
    public static function removeInstanceFromPool($value)
    {
        if (Propel::isInstancePoolingEnabled() && null !== $value) {
            if (is_object($value) && $value instanceof \Schedule) {
                $key = serialize([(null === $value->getScheduleId() || is_scalar($value->getScheduleId()) || is_callable([$value->getScheduleId(), '__toString']) ? (string) $value->getScheduleId() : $value->getScheduleId()), (null === $value->getMentorId() || is_scalar($value->getMentorId()) || is_callable([$value->getMentorId(), '__toString']) ? (string) $value->getMentorId() : $value->getMentorId()), (null === $value->getCustomerId() || is_scalar($value->getCustomerId()) || is_callable([$value->getCustomerId(), '__toString']) ? (string) $value->getCustomerId() : $value->getCustomerId())]);
                ...
                $e = new PropelException("Invalid value passed to removeInstanceFromPool().  Expected primary key or \Schedule object; got " . (is_object($value) ? get_class($value) . ' object.' : var_export($value, true)));
                throw $e;
            }
    ...
    public static function getPrimaryKeyHashFromRow($row, $offset = 0, $indexType = TableMap::TYPE_NUM)
    {
        ...
        if ($row[TableMap::TYPE_NUM == $indexType ? 0 + $offset : static::translateFieldName('ScheduleId', TableMap::TYPE_PHPNAME, $indexType)] === null && $row[TableMap::TYPE_NUM == $indexType ? 3 + $offset : static::translateFieldName('MentorId', TableMap::TYPE_PHPNAME, $indexType)] === null && $row[TableMap::TYPE_NUM == $indexType ? 4 + $offset : static::translateFieldName('CustomerId', TableMap::TYPE_PHPNAME, $indexType)] === null) {
            return null;
        }

        return serialize([(null === $row[TableMap::TYPE_NUM == $indexType ? 0 + $offset : static::translateFieldName('ScheduleId', TableMap::TYPE_PHPNAME, $indexType)] || is_scalar($row[TableMap::TYPE_NUM == $indexType ? 0 + $offset : static::translateFieldName('ScheduleId', TableMap::TYPE_PHPNAME, $indexType)]) || is_callable([$row[TableMap::TYPE_NUM == $indexType ? 0 + $offset : static::translateFieldName('ScheduleId', TableMap::TYPE_PHPNAME, $indexType)], '__toString']) ? (string) $row[TableMap::TYPE_NUM == $indexType ? 0 + $offset : static::translateFieldName('ScheduleId', TableMap::TYPE_PHPNAME, $indexType)] : $row[TableMap::TYPE_NUM == $indexType ? 0 + $offset : static::translateFieldName('ScheduleId', TableMap::TYPE_PHPNAME, $indexType)]), (null === $row[TableMap::TYPE_NUM == $indexType ? 3 + $offset : static::translateFieldName('MentorId', TableMap::TYPE_PHPNAME, $indexType)] || is_scalar($row[TableMap::TYPE_NUM == $indexType ? 3 + $offset : static::translateFieldName('MentorId', TableMap::TYPE_PHPNAME, $indexType)]) || is_callable([$row[TableMap::TYPE_NUM == $indexType ? 3 + $offset : static::translateFieldName('MentorId', TableMap::TYPE_PHPNAME, $indexType)], '__toString']) ? (string) $row[TableMap::TYPE_NUM == $indexType ? 3 + $offset : static::translateFieldName('MentorId', TableMap::TYPE_PHPNAME, $indexType)] : $row[TableMap::TYPE_NUM == $indexType ? 3 + $offset : static::translateFieldName('MentorId', TableMap::TYPE_PHPNAME, $indexType)]), (null === $row[TableMap::TYPE_NUM == $indexType ? 4 + $offset : static::translateFieldName('CustomerId', TableMap::TYPE_PHPNAME, $indexType)] || is_scalar($row[TableMap::TYPE_NUM == $indexType ? 4 + $offset : static::translateFieldName('CustomerId', TableMap::TYPE_PHPNAME, $indexType)]) || is_callable([$row[TableMap::TYPE_NUM == $indexType ? 4 + $offset : static::translateFieldName('CustomerId', TableMap::TYPE_PHPNAME, $indexType)], '__toString']) ? (string) $row[TableMap::TYPE_NUM == $indexType ? 4 + $offset : static::translateFieldName('CustomerId', TableMap::TYPE_PHPNAME, $indexType)] : $row[TableMap::TYPE_NUM == $indexType ? 4 + $offset : static::translateFieldName('CustomerId', TableMap::TYPE_PHPNAME, $indexType)])]);
    }
    ...
    public static function getPrimaryKeyFromRow($row, $offset = 0, $indexType = TableMap::TYPE_NUM)
    {
        ...
        $pks[] = (int) $row[
            $indexType == TableMap::TYPE_NUM
                ? 0 + $offset
                : self::translateFieldName('ScheduleId', TableMap::TYPE_PHPNAME, $indexType)
        ];
        $pks[] = (int) $row[
            $indexType == TableMap::TYPE_NUM
                ? 3 + $offset
     ...
     public static function doDelete($values, ConnectionInterface $con = null)
     {
        ...
        if ($values instanceof Criteria) {
            ...
            foreach ($values as $value) {
                $criterion = $criteria->getNewCriterion(ScheduleTableMap::COL_SCHEDULE_ID, $value[0]);
                $criterion->addAnd($criteria->getNewCriterion(ScheduleTableMap::COL_MENTOR_ID, $value[1]));
                $criterion->addAnd($criteria->getNewCriterion(ScheduleTableMap::COL_CUSTOMER_ID, $value[2]));
    ...
    public static function doInsert($criteria, ConnectionInterface $con = null)
    {
        ...
        if ($criteria->containsKey(ScheduleTableMap::COL_SCHEDULE_ID) && $criteria->keyContainsValue(ScheduleTableMap::COL_SCHEDULE_ID) ) {
            throw new PropelException('Cannot insert a value for auto-increment primary key ('.ScheduleTableMap::COL_SCHEDULE_ID.')');
        }
